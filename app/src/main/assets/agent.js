Java.perform(function () {
    ...
    var tool = {
        ...
        traceClass: function (targetClass) {
            ...
            targets.forEach(function (targetMethod) {
                tool.traceMethod(targetClass + "." + targetMethod);
            });
        },
        libc: {
            dlopen: function () {
    ...
    //user
    //开启日志
    const CRLog = Java.use('com.sec.android.easyMoverCommon.CRLog')
    CRLog.lvl.value = 999
    // CRLog.isPrintingUnitTest.value=true
    CRLog.isEnableStackTrace.value = true
    console.log("[CRLog]enable=true")
    //强制支持备份数据
    const MtpObexDrive = Java.use('com.sec.android.easyMover.OTG.MtpObexDrive')
    const MtpOnlyDrive = Java.use('com.sec.android.easyMover.OTG.MtpOnlyDrive')
    const MakeOption = Java.use('com.sec.android.easyMover.model.ObjApks$MakeOption')
    const AndroidOtgClientService = Java.use('com.sec.android.easyMover.OTG.AndroidOtgClientService')
    const OtgClientManager = Java.use('com.sec.android.easyMover.OTG.OtgClientManager')
    const ApkFileContentManager = Java.use('com.sec.android.easyMover.data.ApkFileContentManager')
    const InstantProperty = Java.use('com.sec.android.easyMover.utility.InstantProperty')
    const ApkBnRHelper = Java.use('com.sec.android.easyMover.data.ApkBnRHelper')
    const IBackupManager = Java.use('android.app.backup.IBackupManager')
    MtpObexDrive._requestApkInfoIsSupportApkData.implementation = function (pkg) {
        console.log("[_requestApkInfoIsSupportApkData]", pkg)
        var ret = this._requestApkInfoIsSupportApkData(pkg)
        console.log("[_requestApkInfoIsSupportApkData]ret true")
        return true
    }
    // AndroidOtgClientService.makeClientInfo.implementation = function () {
    //     console.log("[makeClientInfo]")
    //     var ret = this._requestApkInfoIsSupportApkData(pkg)
    //     console.log("[makeClientInfo]ret:",ret)
    //     return true
    // }
    OtgClientManager.launchApp.implementation = function (js, i) {
        console.log("[launchApp]", js)
        var ret = this.launchApp(js, i)
        console.log("[launchApp]ret:", ret)
        return ret
    }
    MtpOnlyDrive.requestBackup.implementation = function (objItems, objApks, cbifdrivemsg) {
        console.log("[requestBackup]", objItems.toJson())
        console.log("[requestBackup]", objApks.toJson(MakeOption.Full))
        var ret = this.requestBackup(objItems, objApks, cbifdrivemsg)
        console.log("[requestBackup]ret:", ret.getPath())
        return ret
    }
    // ApkFileContentManager.getAppData.implementation = function (_1,_2,_3) {
    //     console.log("[getAppData]",objItems.toJson())
    //      var ret = this.requestBackup(_1,_2,_3)
    //     console.log("[getAppData]ret:",ret.getPath())
    //     return ret
    // }
    tool.traceMethod("com.sec.android.easyMover.data.ApkFileContentManager.getAppData",false)
    tool.traceMethod("com.sec.android.easyMover.data.ApkBnRHelper.processBnRPackage",false)
    tool.traceMethod("android.app.backup.IBackupManager.setDefaultImpl",false)
    tool.traceMethod("com.samsung.android.SSPHost.SyncBackupManager.backupApk",true)
    console.log('[init frida ok]')
});