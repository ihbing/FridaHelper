...
import com.ihbing.fridahelper.Shell;
import com.ihbing.fridahelper.ShellAsync;
...
//                " -s " + agentPath + " --runtime=v8 -e");
      shellAsync=  new ShellAsync(mInjector.getPath() + " -n " + packageName + " -s " + agentPath + " --runtime=v8", true, new ShellAsync.OnShellAsyncListener() {
            @Override
            public void onReadLine(String line) {
                if (!TextUtils.isEmpty(line)) {
                    try {
                        ...
                        if ("send".equals(type)) {
                            EventBus.getDefault().post(new FridaMessage(10000, payload));
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
                        EventBus.getDefault().post(new FridaMessage(10000, line));
                    }
                }
          ...
          public void onStopped() {
              EventBus.getDefault().post(new FridaMessage(10000, "已停止"));
          }

      });
        Go.go(() -> shellAsync.start());
